...
let PolytopeViewer = function (container) {
    ...
    function makeFacetTable(geometry) {
        ...
        for (let i = 0; i < faces.length; i++) {
            ...
            for (let j = 0; j < table.length; j++) {
                if (face.normal.distanceTo(table[j][0].normal)<Math.pow(10, -7)) {
                    table[j].push(face);
                    added = true;
                }
            }
    ...
    self.setPolytope = function (vertices, edges, facets) {
        ...
        facetObjects = [];
        //getFacetAbstract(facetTable[i][0],facets),
        facetListDebug = facetTable;
        facetFunctionDebug = getFacetAbstract;
        facetDebug = facets;
        for(let i=0;i<facetTable.length;i++) {
            facetObjects.push(
                {   //TODO: bugfix, getFacetAbstract returns undefined
                    abstract: {facet: getFacetAbstract(facetTable[i][0],facets),
                    //abstract: {
						color: '0x999009'},
                    mesh: facetTable[i],
                    object_type: 'facet',
